...
/** Best-effort geolocation; resolves null if denied/unavailable within 8s. */
...
export function SosPanel() {
  ...
  function fireSos() {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) setError(res.error ?? "Could not send SOS.");
      else {
  ...
  function submitCategory() {
    ...
    setError(null);
    startTransition(async () => {
      const [coords, photoUrl] = await Promise.all([
        getLocation(),
      ...
      if (!res.ok) setError(res.error ?? "Could not send report.");
      else {
        setSentCoords(coords);
        setSent(type);
        setDialog(null);
  ...
  if (sent) {
    return (
      <div className="rounded-xl border-2 border-green-500/40 bg-green-50 p-6 text-center">
        ...
        {sentCoords && (
          <p className="mt-4 inline-flex items-center gap-1 text-xs text-green-700">
            <MapPin className="h-3.5 w-3.5" />
            {sentCoords.lat.toFixed(5)}, {sentCoords.lng.toFixed(5)}
          </p>
        )}
        <div className="mt-6">
          <Button variant="outline" onClick={() => { setSent(null); setSentCoords(null); }}>
            Back to safety home
          </Button>
        </div>
      </div>
