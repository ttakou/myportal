...
import { assigneeLabel, parseAssignee, parseFuel, poolFirst, vehicleLabel } from "@/lib/transport/vehicles";
...
describe("vehicle label", () => {
  it("names the vehicle, its plate and who it is assigned to", () => {
    ...
    expect(vehicleLabel({ name: "Toyota Prado", plate: "CE.303.MS", assigned_to: "Operations Manager", holder_name: "Paul Wambo" })).toBe(
      "Toyota Prado · CE.303.MS (Operations Manager · Paul Wambo)",
    );
    expect(assigneeLabel({ assigned_to: null, holder_name: "Paul Wambo" })).toBe("Paul Wambo");
    expect(assigneeLabel({ assigned_to: null, holder_name: null })).toBeNull();
